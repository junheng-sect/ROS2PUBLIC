...
class PidTuningV3CsvLoggerNode(Node):
    """记录调参过程到 CSV，并在退出时输出摘要指标。"""

    def __init__(self):
        super().__init__('pid_tuning_v3_csv_logger_node')
        ...
        self.declare_parameter('file_prefix', 'pid_tuning_v3')
        self.declare_parameter(
            'summary_csv_path',
            '/home/zjh/project/rasip_pi_ws/log/tracking_csv/pid_tuning_v3_summary.csv',
        )
        ...
        self.param_snapshot = {
            ...
            'require_offboard': int(bool(self.get_parameter('require_offboard').value)),
    ...
    def _write_row(self):
        ...
        # 指标仍按 OFFBOARD + aruco fresh 统计，便于和 v2 横向比较。
    ...
    def _compute_metrics(self):
        ...
        exy = [math.hypot(ex_marker[idx], ey_marker[idx]) for idx in range(len(ex_marker))]
    ...
    def _print_summary(self, metrics):
        """在终端打印本次调参摘要。"""
        self.get_logger().info('===== pid_tuning_v3 调参指标汇总（OFFBOARD+fresh） =====')
        self.get_logger().info(
