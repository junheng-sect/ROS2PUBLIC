...
class PIDController:
    ...
    def update(self, error: float) -> float:
        ...
        # 首次 update 时不构造假导数，避免 reset 后因 prev_error=0 产生 D kick。
...
class PidTuningV5Node(Node):
    """BODY_NED 控制节点：在 v4 基础上增加 D kick 修正与 OFFBOARD 平滑切入."""

    def __init__(self):
        super().__init__('pid_tuning_v5_node')
        ...
        # v5 中 XY 最终限幅统一由 vxy_limit 控制；vx_limit / vy_limit 仅保留兼容参数入口。
        self.vxy_limit = abs(float(self.get_parameter('vxy_limit').value))
        ...
        self.offboard_entry_ramp_sec = float(
            self.get_parameter('offboard_entry_ramp_sec').value
        )

        # ===== PID 控制器 =====
        # XY 的最终限幅改为“先算向量、再按合速度等比例缩放”，
        # 因此这里不再使用分轴输出限幅，避免单轴先撞限后把速度方向掰偏。
        ...
        self.offboard_entry_time: Optional[float] = None
        ...
        self.get_logger().info(
            '速度限幅 | '
            f'vxy_limit={self.vxy_limit:.3f} (XY 合速度唯一生效限幅) | '
            f'vx_limit={self.format_optional_limit(self.vx_limit)} '
            '(兼容保留，不参与控制) | '
            f'vy_limit={self.format_optional_limit(self.vy_limit)} '
            f'(兼容保留，不参与控制) | vz_limit={self.vz_limit:.3f}'
        )
        self.get_logger().info(
            '参数来源 | '
        ...
        self.get_logger().info(
            'OFFBOARD 平滑切入参数 | '
            f'offboard_entry_ramp_sec={self.offboard_entry_ramp_sec:.3f}s'
        )
    ...
    @staticmethod
    def format_optional_limit(limit_value: float) -> str:
        """把兼容保留参数格式化为日志文本，NaN 时直接显示 nan."""
        return f'{limit_value:.3f}' if math.isfinite(limit_value) else 'nan'

    @staticmethod
    def limit_xy_velocity_preserve_direction(
        vx_raw: float,
        vy_raw: float,
        vxy_limit: float,
    ) -> tuple[float, float, float, float, bool]:
        """
        按 XY 合速度统一限幅，并保持速度方向不变.

        返回：
        - vx_limited / vy_limited：限幅后的 XY 指令
        - vxy_raw：限幅前合速度
        - xy_limit_scale：本次缩放系数
        - xy_limit_active：本次是否触发了合速度限幅
        """
        vxy_raw = math.hypot(vx_raw, vy_raw)
        if vxy_raw <= 1e-9:
            return vx_raw, vy_raw, 0.0, 1.0, False

        if vxy_limit <= 0.0:
            return 0.0, 0.0, vxy_raw, 0.0, True

        if vxy_raw <= vxy_limit:
            return vx_raw, vy_raw, vxy_raw, 1.0, False

        xy_limit_scale = vxy_limit / vxy_raw
        return (
            xy_limit_scale * vx_raw,
            xy_limit_scale * vy_raw,
            vxy_raw,
            xy_limit_scale,
            True,
        )
    ...
    @staticmethod
    def apply_xy_deadband(
        vx_cmd: float,
        vy_cmd: float,
        deadband: float,
    ) -> tuple[float, float, bool]:
        """
        对 XY 合速度统一做死区处理，避免单轴死区再次破坏方向.

        当最终 XY 合速度幅值小于 deadband 时，X/Y 一起压到 0。
        """
        if math.hypot(vx_cmd, vy_cmd) < deadband:
            return 0.0, 0.0, True
        return vx_cmd, vy_cmd, False
    ...
    def get_offboard_ramp_state(
        self,
        now_sec: Optional[float] = None,
    ) -> tuple[bool, float]:
        """返回 OFFBOARD 平滑接管窗口是否激活以及当前斜坡系数."""
        if self.offboard_entry_ramp_sec <= 0.0:
            return False, 1.0
        ...
        alpha = max(
            ...
            min(
                1.0,
                (current_time - self.offboard_entry_time) / self.offboard_entry_ramp_sec,
            ),
        )
        return alpha < 1.0, alpha

    @staticmethod
    def format_ramp_status(
        ramp_active: bool,
        ramp_alpha: float,
        output_scaled: bool,
    ) -> str:
        """格式化 OFFBOARD 平滑接管状态，便于 latest_status 直接复用."""
        return (
            f'OFFBOARD斜坡窗口={"是" if ramp_active else "否"} | '
            f'ramp_alpha={ramp_alpha:.3f} | '
            f'输出缩放={"是" if output_scaled else "否"}'
        )
    ...
    def control_loop(self):
        ...
        now_sec = time.time()
        offboard_active = self.current_state.mode == 'OFFBOARD'
        if offboard_active and not self.was_offboard:
            self.reset_all_pids()
            if self.offboard_entry_ramp_sec > 0.0:
                self.offboard_entry_time = now_sec
                self.get_logger().info(
                    '检测到 OFFBOARD 上升沿，已 reset PID，已进入斜坡接管阶段 | '
                    f'offboard_entry_ramp_sec={self.offboard_entry_ramp_sec:.3f}s'
                )
            else:
                self.offboard_entry_time = None
                self.get_logger().info(
                    '检测到 OFFBOARD 上升沿，已 reset PID，斜坡接管按配置关闭 | '
                    f'offboard_entry_ramp_sec={self.offboard_entry_ramp_sec:.3f}s'
                )
        elif not offboard_active:
            self.offboard_entry_time = None

        ramp_active, ramp_alpha = self.get_offboard_ramp_state(now_sec)

        if self.require_offboard and not offboard_active:
            self.reset_all_pids()
            self.publish_body_velocity(0.0, 0.0, 0.0, 0.0)
            self.latest_status = (
                f'模式={self.current_state.mode}，未进入 OFFBOARD，输出零速 | '
                f'{self.format_ramp_status(ramp_active, ramp_alpha, False)}'
            )
            self.was_offboard = offboard_active
            return
        ...
        # XY 先保留原始 PID 方向，再统一按合速度限幅。
        # 这样在你主用的 kp_xy=1.0、kd_xy=0.06、vxy_limit=0.60 这类配置下，
        # 即使某一轴误差更大，最终速度方向也仍尽量指向当前目标误差方向。
        vx_pid_raw = self.pid_x.update(ex_body)
        vy_pid_raw = self.pid_y.update(ey_body)
        vx_pid_limited, vy_pid_limited, vxy_raw, xy_limit_scale, xy_limit_active = (
            self.limit_xy_velocity_preserve_direction(
                vx_pid_raw,
                vy_pid_raw,
                self.vxy_limit,
            )
        )
        # XY 死区放在“保方向限幅”之后，对合速度统一处理，避免再把方向掰偏。
        vx_pid, vy_pid, xy_deadband_active = self.apply_xy_deadband(
            vx_pid_limited,
            vy_pid_limited,
            self.velocity_deadband,
        )
        vxy_cmd = math.hypot(vx_pid, vy_pid)
        ...
        if ramp_active:
            vx = ramp_alpha * vx_pid
            vy = ramp_alpha * vy_pid
            vz = ramp_alpha * vz_pid
            output_scaled = True
        else:
            vx = vx_pid
            vy = vy_pid
        ...
        self.latest_status = (
            ...
            f'{z_status} | '
            f'vx_pid_raw={vx_pid_raw:.3f}, vy_pid_raw={vy_pid_raw:.3f} | '
            f'vxy_raw={vxy_raw:.3f}, vxy_cmd={vxy_cmd:.3f}, '
            f'xy_limit_scale={xy_limit_scale:.3f}, '
            f'xy_limit_active={"true" if xy_limit_active else "false"}, '
            f'xy_deadband_active={"true" if xy_deadband_active else "false"} | '
            f'vx_pid={vx_pid:.3f}, vy_pid={vy_pid:.3f}, vz_pid={vz_pid:.3f} | '
            f'vx={vx:.3f}, vy={vy:.3f}, vz={vz:.3f}, yaw_rate={yaw_rate:.3f} | '
            f'{self.format_ramp_status(ramp_active, ramp_alpha, output_scaled)}'
        )
        self.was_offboard = offboard_active
