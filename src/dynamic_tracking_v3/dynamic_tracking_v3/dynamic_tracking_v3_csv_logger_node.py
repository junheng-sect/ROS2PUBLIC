...
class DynamicTrackingV3CsvLoggerNode(Node):
    ...
    def __init__(self):
        super().__init__('dynamic_tracking_v3_csv_logger_node')
        ...
        self.declare_parameter('file_prefix', 'dynamic_tracking_v3')
        self.declare_parameter(
            'summary_csv_path',
            '/home/zjh/project/rasip_pi_ws/log/tracking_csv/'
        ...
        self.param_snapshot = {
            ...
            'enable_z_hold': int(
                self.parameter_as_bool(self.get_parameter('enable_z_hold').value)
            ),
    ...
    def _print_summary(self, metrics):
        """在终端打印本次调参摘要."""
        self.get_logger().info(
            '===== dynamic_tracking_v3 调参指标汇总（OFFBOARD） ====='
        )
        self.get_logger().info(
