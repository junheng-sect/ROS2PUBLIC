#!/usr/bin/env python3
"""
ROS 2 ArUco 检测节点（OpenCV 4.10 兼容版）
- 修复 drawDetectedMarkers 格式错误
- Z > 0.5m：使用外层码（ID 33）
- Z <= 0.5m：使用内层码（ID 42）
- 带滞回切换，避免频繁跳变
"""
...
class ArucoDetectorNode(Node):
    def __init__(self):
        super().__init__('aruco_threshold')
    ...
    def _convert_corners_for_drawing(self, corners):
        """
        转换 corners 格式以兼容 drawDetectedMarkers
        新 API: (4,1,2) → 旧格式: (4,2)
        """
        if corners is None:
            return None
        ...
        for c in corners:
            if c.ndim == 3 and c.shape[1] == 1:
                converted.append(c.reshape(4, 2))
            else:
                converted.append(c)
        return converted
    ...
    def listener_callback(self, msg):
        ...
        if ids is not None and len(ids) > 0:
            ...
            if len(marker_poses) > 0:
                ...
                corners_selected = pose_info['corners']
                ...
                try:
                    ...
                    self.get_logger().debug(f'drawAxis warning: {e}')
            ...
            try:
                # 转换 corners 格式以兼容 drawDetectedMarkers
                corners_for_draw = self._convert_corners_for_drawing(corners)
                cv2.aruco.drawDetectedMarkers(cv_image, corners_for_draw, ids)
            except Exception as e:
                self.get_logger().warn(f'drawDetectedMarkers failed: {e}')
                # 降级：手动绘制标记框
        ...
        cv2.putText(cv_image, switch_text, (10, 40),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.3, (200, 200, 200), 1)
        
        perf_text = f'Frames: {self.frame_count} | Detects: {self.detect_count}'
        cv2.putText(cv_image, perf_text, (10, 20),
