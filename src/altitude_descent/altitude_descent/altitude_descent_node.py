...
class AltitudeDescentNode(Node):
    ...
    def __init__(self) -> None:
        super().__init__('altitude_descent_node')
        ...
        self.declare_parameter('status_topic', '/altitude_descent/status')
        ...
        self.get_logger().info(
            ...
            f'min_height_stop={self.min_height_stop:.3f}m | '
    ...
    def _ensure_descent_trajectory(self, distance_m: float) -> None:
        """首次有效控制时用当前测距高度作为轨迹起点."""
        if self.trajectory_active:
            return
        self.trajectory_active = True
        self.trajectory_start_z = float(distance_m)
        self.trajectory_start_time_sec = self._now_sec()
        self.last_reference_z = float(distance_m)
    ...
    def _control_loop(self) -> None:
        ...
        if self.min_height_stop > 0.0 and distance_m <= self.min_height_stop:
            self.last_low_height_stop_active = True
            self._publish_zero_and_status(
                distance_m,
                distance_age_sec,
                (
                    f'测距高度 {distance_m:.3f}m <= '
                    f'min_height_stop {self.min_height_stop:.3f}m，停止下降'
                ),
            )
            return

        self._ensure_descent_trajectory(distance_m)
        reference_z = self._update_reference_z()
        self.last_height_error_m = reference_z - distance_m
