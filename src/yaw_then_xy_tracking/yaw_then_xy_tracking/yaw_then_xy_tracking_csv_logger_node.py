...
class YawThenXYTrackingCsvLoggerNode(Node):
    """Yaw-Then-XY 记录节点：写运行 CSV，并在结束时输出指标与汇总。"""

    def __init__(self):
        super().__init__('yaw_then_xy_tracking_csv_logger_node')
        ...
        self.declare_parameter('file_prefix', 'yaw_then_xy_tracking')
        self.declare_parameter(
            'summary_csv_path',
            '/home/zjh/project/rasip_pi_ws/log/tracking_csv/yaw_then_xy_tracking_summary.csv',
        )
        # ===== 结束后自动同步 =====
        # 说明：
        ...
        # 指标统计窗口：仅统计 OFFBOARD 最后 N 秒，避免初始状态差异影响评估。
        self.declare_parameter('metric_window_sec', 5.0)
        ...
        self.declare_parameter('vxy_limit', 1.0)
        ...
        self.metric_window_sec = float(self.get_parameter('metric_window_sec').value)
        ...
        self.get_logger().info(f'Yaw-Then-XY CSV记录已启动，输出文件: {self.csv_path}')
    ...
    def _compute_metrics(self):
        # 统计“OFFBOARD 最后 N 秒”窗口：先保留全 OFFBOARD，再截取末尾时间窗。
        off_rows = [r for r in self.metric_rows if r['mode'] == 'OFFBOARD']
        if len(off_rows) == 0:
            return {
                'eval_rows': 0,
                'offboard_rows_total': 0,
                'offboard_rows_window': 0,
        ...
        t_end = off_rows[-1]['t']
        t_start = t_end - self.metric_window_sec
        off_rows_window = [r for r in off_rows if r['t'] >= t_start]
        eval_rows = [r for r in off_rows_window if r['aruco_fresh'] == 1]

        result = {
            'eval_rows': len(eval_rows),
            'offboard_rows_total': len(off_rows),
            'offboard_rows_window': len(off_rows_window),
            'metric_window_sec': self.metric_window_sec,
            'status': 'ok' if len(eval_rows) >= 1 else 'insufficient_data',
        }

        if len(off_rows_window) == 0:
            result.update({'fresh_ratio': float('nan'), 'max_stale_s': float('nan')})
            return result

        fresh_ratio = len(eval_rows) / len(off_rows_window)

        # OFFBOARD 内最长连续 stale 时长。
        stale_run = 0
        max_stale_run = 0
        dt_vals = []
        for i in range(1, len(off_rows_window)):
            dt_vals.append(max(0.0, off_rows_window[i]['t'] - off_rows_window[i - 1]['t']))
        mean_dt = sum(dt_vals) / len(dt_vals) if dt_vals else 0.0
        for r in off_rows_window:
            if r['aruco_fresh'] == 0:
                stale_run += 1
    ...
    def _append_summary_csv(self, metrics):
        ...
        row = {
            ...
            'offboard_rows_total': metrics.get('offboard_rows_total', 0),
            'offboard_rows_window': metrics.get('offboard_rows_window', 0),
            'metric_window_sec': metrics.get('metric_window_sec', self.metric_window_sec),
    ...
    def _print_summary(self, metrics):
        self.get_logger().info('===== Yaw-Then-XY 指标汇总（OFFBOARD最后5s+fresh） =====')
        self.get_logger().info(
            f"status={metrics.get('status')} | offboard_total={metrics.get('offboard_rows_total')} | "
            f"offboard_window={metrics.get('offboard_rows_window')} | eval_rows={metrics.get('eval_rows')} | "
...
def main(args=None):
    ...
    node = YawThenXYTrackingCsvLoggerNode()
    try:
        rclpy.spin(node)
