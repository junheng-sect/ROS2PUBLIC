...
class ArucoDetectorNode(Node):
    def __init__(self):
        ...
        # 标记尺寸参数
        self.declare_parameter('marker_size_33', 0.135)  # 外层码
        self.declare_parameter('marker_size_42', 0.017)  # 内层码
        
        # 【新增】切换阈值参数
        self.declare_parameter('switch_distance', 0.5)   # 切换距离（米）
        self.declare_parameter('switch_hysteresis', 0.05) # 滞回区间（米），避免频繁切换
        ...
        # 【新增】标记切换状态
        self.current_marker_id = 33  # 默认使用外层码
    ...
    def _select_marker_id(self, z_value):
        ...
        滞回区间说明：
        - 当前使用外层码 (33)：Z < 0.45m 时才切换到内层码
        - 当前使用内层码 (42)：Z > 0.55m 时才切换到外层码
        - 中间区域保持当前标记，避免频繁切换
        """
    ...
    def listener_callback(self, msg):
        ...
        detected_count = 0
        best_pose = None  # 存储最佳位姿
        
        if ids is not None:
            detected_ids = ids.flatten().tolist()
            
            # ========== 策略：同时检测两种标记，根据距离选择 ==========
            marker_poses = {}  # 存储每个 ID 的位姿信息
