...
class PIDController:
    """简单 PID 控制器，带积分限幅、输出限幅和异常 dt 保护."""
    ...
    def update(self, error: float) -> float:
        ...
        if not self.initialized or self.prev_time is None:
            ...
            self.prev_time = now
            return self._limit_output(self.kp * float(error))

        dt = now - self.prev_time
        if dt <= 1e-6 or dt > 1.0 or not math.isfinite(dt):
            # dt 异常时只保留比例项，避免微分项产生尖峰。
            self.prev_error = float(error)
            self.prev_time = now
            return self._limit_output(self.kp * float(error))

        p_term = self.kp * float(error)

        # 积分项用于消除稳态误差，积分限幅用于避免长时间饱和。
        self.integral += float(error) * dt
        self.integral = max(-self.i_limit, min(self.i_limit, self.integral))
        i_term = self.ki * self.integral

        # 微分项根据误差变化率抑制高度过冲。
        d_term = self.kd * (float(error) - self.prev_error) / dt

        self.prev_error = float(error)
        self.prev_time = now
        return self._limit_output(p_term + i_term + d_term)
...
class AltitudeHoldNode(Node):
    """基于测距仪的定高悬停节点，只控制 Z 速度，XY 与 yaw 固定为零."""
    ...
    def __init__(self) -> None:
        ...
        self.declare_parameter('status_topic', '/altitude_hold/status')
        ...
        self.get_logger().info(
            '电机信号 | '
            f'motor_signal_source={self.motor_signal_source} | '
            f'esc_status={self.esc_status_topic} | '
            f'esc_telemetry={self.esc_telemetry_topic} | '
            f'rc_out={self.rc_out_topic}'
        )
    ...
    def _is_fresh(self, stamp_time, timeout_sec: float) -> bool:
        """判断缓存样本是否仍在有效时间窗口内."""
        return self._age_sec(stamp_time) <= float(timeout_sec)
    ...
    def _publish_body_velocity(
        ...
        """发布 BODY_NED 速度指令，保持与现有控制包的符号约定一致."""
        msg = PositionTarget()
    ...
    def _control_loop(self) -> None:
        ...
        if self.require_offboard and self.current_state.mode != 'OFFBOARD':
            ...
            self._publish_status(distance_m, distance_age_sec)
            return
        ...
        if not valid:
            ...
            self.last_height_error_m = float('nan')
            self._publish_body_velocity(0.0, 0.0, 0.0, 0.0)
            self.latest_status = f'{distance_status}，输出零速并重置 PID_Z'
            self._publish_status(distance_m, distance_age_sec)
            return

        self.last_height_error_m = self.target_z - distance_m
        vz = self.pid_z.update(self.last_height_error_m)
