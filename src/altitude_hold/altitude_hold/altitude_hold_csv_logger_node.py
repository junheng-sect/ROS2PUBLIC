...
class AltitudeHoldCsvLoggerNode(Node):
    """独立 CSV logger，记录定高控制与电机/油门观测信号."""

    def __init__(self) -> None:
        super().__init__('altitude_hold_csv_logger_node')
        ...
        self.declare_parameter(
            'output_dir',
            '/home/zjh/project/rasip_pi_ws/log/altitude_hold_csv',
        )
        self.declare_parameter('file_prefix', 'altitude_hold')
        ...
        self.get_logger().info('altitude_hold_csv_logger_node 已启动')
    ...
    def _on_status(self, msg: String) -> None:
        ...
        try:
            ...
            self.get_logger().warn(f'解析 altitude_hold/status 失败: {exc}')
    ...
    def _write_row(self) -> None:
        ...
        if status_fresh:
            ...
            height_error = (
                float(target_z) - distance_z
                if distance_valid else float('nan')
            )
            distance_status = ''
            cmd_vx = float('nan')
            cmd_vy = float('nan')
...
def main(args=None) -> None:
    """启动 altitude_hold_csv_logger_node."""
    rclpy.init(args=args)
    node = AltitudeHoldCsvLoggerNode()
    try:
        rclpy.spin(node)
