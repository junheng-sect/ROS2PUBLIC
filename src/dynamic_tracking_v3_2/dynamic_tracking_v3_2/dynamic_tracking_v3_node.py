...
class PIDController:
    ...
    def _now(self) -> Time:
        ...
        return rclpy.clock.Clock(clock_type=ClockType.SYSTEM_TIME).now()
    ...
    def update(self, error: float) -> float:
        """根据当前误差计算控制输出."""
        now = self._now()
        if not self._initialized or self.prev_time is None:
...
class AngleLowPassFilter:
    ...
    def update(self, raw_angle: float, tau_sec: float) -> float:
        ...
        now = self._now()
        raw_angle = wrap_to_pi(float(raw_angle))
        tau_sec = float(tau_sec)
        if not self.initialized or self.prev_time is None:
            self.initialized = True
        ...
        angle_error = wrap_to_pi(raw_angle - self.value)
        alpha = dt / (tau_sec + dt)
        self.value = wrap_to_pi(self.value + alpha * angle_error)
        return self.value
...
class SlewRateLimiter:
    ...
    def __init__(self, initial_value: float = 0.0, *, clock=None):
        self._clock = clock
...
class DynamicTrackingV3Node(Node):
    ...
    def __init__(self):
        super().__init__('dynamic_tracking_v3_2_node')
        ...
        # yaw_rate 输出符号反转，用于补偿视觉 yaw 与 BODY_NED yaw_rate 正方向之间的差异。
        ...
        self.invert_yaw_rate_output = parameter_as_bool(
        ...
        self.require_offboard = parameter_as_bool(
            self.get_parameter('require_offboard').value
        )
        self.enable_z_hold = parameter_as_bool(
            self.get_parameter('enable_z_hold').value
        )
        self.enable_z_yaw_smoothing = parameter_as_bool(
        ...
        # 对控制器注入 ROS 单调时钟，避免 wall time 跳变导致 dt 异常。
        node_clock = self.get_clock()

        # ===== PID 控制器 =====
        self.pid_x = PIDController(
            self.kp_x, self.ki_x, self.kd_x,
            out_limit=None, clock=node_clock,
        )
        ...
        self.pid_z = PIDController(
            ...
            out_limit=self.vz_limit, clock=node_clock,
        )
        self.pid_yaw = PIDController(
            self.kp_yaw, self.ki_yaw, self.kd_yaw,
            out_limit=self.yaw_rate_limit, clock=node_clock,
        )

        # ===== Z / yaw 平滑器 =====
        self.z_lpf = ScalarLowPassFilter(clock=node_clock)
        self.yaw_lpf = AngleLowPassFilter(clock=node_clock)
        self.vz_slew_limiter = SlewRateLimiter(clock=node_clock)
        self.yaw_rate_slew_limiter = SlewRateLimiter(clock=node_clock)

        # 用于仅在平滑开关状态变化时触发重置，避免每周期无效 reset 调用。
        self._was_smoothing_enabled = self.enable_z_yaw_smoothing
        ...
        self.pose_sub = self.create_subscription(
            ArucoBasePose, self.pose_topic, self.pose_callback, 10,
        )
        self.setpoint_pub = self.create_publisher(
            PositionTarget, self.setpoint_raw_topic, 10,
        )
        self.pipeline_timing_pub = self.create_publisher(
            PipelineTiming, '/debug/pipeline_timing', 10,
        )
        ...
        self.get_logger().info(
            'dynamic_tracking_v3_2_node 已启动 | '
        ...
        self.get_logger().info(
            '改进项 vs v3 | ROS 单调时钟注入、PID D-term 保护、公共函数抽取、'
            'yaw_rate 符号参数化、平滑器按需重置'
        )
    ...
    def publish_pipeline_timing(
        self,
        timing_pose_msg: ArucoBasePose,
        ctrl_cb_start_stamp,
        setpoint_pub_stamp,
    ):
        """发布完整视觉管线 timing，供 CSV logger 记录真实节点内部时间点."""
    ...
    def _maybe_reset_smoothing_on_toggle(self):
        """仅在平滑开关状态变化时重置平滑器，避免每周期无效 reset."""
        if self.enable_z_yaw_smoothing != self._was_smoothing_enabled:
            if not self.enable_z_yaw_smoothing:
                self.reset_all_smoothing()
            self._was_smoothing_enabled = self.enable_z_yaw_smoothing
    ...
    def control_loop(self):
        ...
        if self.enable_z_yaw_smoothing:
            ...
            yaw_rel_corrected = self.yaw_lpf.update(
                yaw_rel_corrected_raw, self.yaw_lpf_tau_sec,
            )
        else:
            self.z_lpf.reset()
            self.yaw_lpf.reset()
        ...
        # — Yaw 轴控制 —
        eyaw = wrap_to_pi(self.target_yaw - yaw_rel_corrected)
        if self.enable_z_yaw_smoothing:
        ...
        if self.enable_z_yaw_smoothing:
            yaw_rate = self.yaw_rate_slew_limiter.update(
                yaw_rate, self.yaw_rate_slew_rate_limit,
            )
        else:
