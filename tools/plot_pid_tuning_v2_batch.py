...
def load_summary(summary_csv_path: str, csv_dir: str) -> Dict[str, SummaryRow]:
    """读取 summary，并只保留当前目录下实验对应的记录。"""
    csv_names = {
        file_name
        for file_name in os.listdir(csv_dir)
        if file_name.endswith(".csv") and not file_name.endswith("_summary.csv")
    }
...
def save_setpoint_timeseries_plot(
    ...
    vz_limit = summary.vz_limit if not math.isnan(summary.vz_limit) else 0.5
...
def save_velocity_vector_plot(
    ...
    """在原始 aruco_x/aruco_y 轨迹图上绘制速度矢量，并统计其与 target 的对齐度。"""
    valid_mask = (
        (data["mode"] == "OFFBOARD")
        & np.isfinite(data["aruco_x"])
        & np.isfinite(data["aruco_y"])
        ...
        & np.isfinite(data["camera_yaw_compensation_deg"])
    )
    ...
    sampled_comp_deg = data["camera_yaw_compensation_deg"][sampled_indices]

    # 还原内部 FLU 机体系速度：logger 记录的是 BODY_NED 消息值，y 需要先取反。
    vx_body = sampled_sp_vx
    vy_body = -sampled_sp_vy

    # 再逆补偿回原始视觉平面误差坐标系。
    theta = np.deg2rad(sampled_comp_deg)
    cos_theta = np.cos(-theta)
    sin_theta = np.sin(-theta)
    vx_raw_ctrl = cos_theta * vx_body - sin_theta * vy_body
    vy_raw_ctrl = sin_theta * vx_body + cos_theta * vy_body

    # 原始控制误差使用 ey_raw = aruco_y - target_y，映射回图坐标时需要再次翻转 y。
    vx_plot = vx_raw_ctrl
    vy_plot = -vy_raw_ctrl
    ...
    if sampled_comp_deg.size:
        text_lines.append(f"camera_comp={sampled_comp_deg[-1]:.1f} deg")
    axis.text(
        0.02,
        0.98,
        "\n".join(text_lines),
...
def write_run_summary(
    ...
    compensation = data["camera_yaw_compensation_deg"][~np.isnan(data["camera_yaw_compensation_deg"])]
    ...
    lines = [
        ...
        f"camera_yaw_compensation_deg: {compensation[-1]:.3f}" if compensation.size else "camera_yaw_compensation_deg: nan",
...
def build_overall_report(summary_by_name: Dict[str, SummaryRow], output_path: str) -> None:
    """输出本批实验总体分析文件。"""
    ...
    ranked_by_xy = sorted(ok_rows, key=lambda item: (math.isnan(item[1].rmse_xy), item[1].rmse_xy))
    lines = [
        "# 4.5.2 pid_tuning_v2 数据分析",
        "",
        f"- 总实验数：{len(rows)}",
...
def main() -> None:
    """程序入口。"""
    parser = argparse.ArgumentParser(description="批量分析 pid_tuning_v2 CSV 并生成图片")
    parser.add_argument("--csv-dir", required=True, help="待分析 CSV 目录")
    parser.add_argument("--summary-csv", required=True, help="pid_tuning_v2_summary.csv 路径")
    ...
    build_overall_report(summary_by_name, os.path.join(args.output_dir, "analysis_summary.md"))
