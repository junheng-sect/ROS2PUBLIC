...
class DynamicTrackingCsvLoggerNode(Node):
    """PID 调参记录节点：写运行 CSV，并在结束时输出指标与汇总。"""

    def __init__(self):
        super().__init__('dynamic_tracking_csv_logger_node')
        ...
        self.declare_parameter('file_prefix', 'dynamic_tracking')
        self.declare_parameter('summary_csv_path', '/home/zjh/project/rasip_pi_ws/log/tracking_csv/dynamic_tracking_summary.csv')
        # ===== 结束后自动同步 =====
        # 说明：
        ...
        self.get_logger().info(f'动态跟踪CSV记录已启动，输出文件: {self.csv_path}')
    ...
    def _print_summary(self, metrics):
        self.get_logger().info('===== 动态跟踪指标汇总（OFFBOARD+fresh） =====')
...
def main(args=None):
    ...
    node = DynamicTrackingCsvLoggerNode()
    try:
        rclpy.spin(node)
