...
class ArucoDetectorNode(Node):
    def __init__(self):
        super().__init__('aruco_csv')
        
        # ========== 参数声明 ==========
        self.declare_parameter('image_topic', '/image_raw')
        self.declare_parameter('use_compressed', False)  # 默认关闭压缩
        ...
        self.declare_parameter('csv_log_path', os.path.expanduser('~/project/project_ws/src/aruco_csv/aruco_log/aruco_detection_log.csv'))
        ...
        self.subscription = self.create_subscription(
            self.msg_type, self.subscribe_topic, self.listener_callback, qos)
        ...
        self.dist_coeffs = np.array([[0.106, -0.277, -0.002, 0.001, 0.0]], dtype=np.float32)

        # 初始化 CSV
        self._init_csv_log()
        
        # 日志
        self.get_logger().info(f'📷 ArUco CSV Node started | Sub: {self.subscribe_topic} | CSV: {self.csv_path}')
        self.frame_count = 0
        self.detect_count = 0
        self.switch_count = 0
    ...
    def _init_csv_log(self):
        ...
        try:
            ...
            # 添加启动时间戳：aruco_detection_log_20260221_215203.csv
    ...
    def _write_csv_log(self, ts, mid, x, y, z, yaw):
        if not self.enable_csv_log or not self.csv_writer:
            return
        try:
            self.csv_writer.writerow([ts, mid, f'{x:.6f}', f'{y:.6f}', f'{z:.6f}', f'{yaw:.6f}'])
            self.csv_file.flush()
        except Exception as e:
        ...
        if ids is not None:
            for i in range(len(ids)):
                ...
                rvec, tvec, _ = aruco.estimatePoseSingleMarkers(
                    corners[i], marker_len, self.camera_matrix, self.dist_coeffs)
                ...
                pose_msg.roll, pose_msg.pitch, pose_msg.yaw = float(euler[0]), float(euler[1]), float(yaw)
                self.pose_publisher.publish(pose_msg)
                
                # CSV 记录
                ts = datetime.now().strftime('%Y-%m-%d %H:%M:%S.%f')[:-3]
                self._write_csv_log(ts, int(marker_id), t[0], t[1], t[2], yaw)
                
                # 绘制坐标轴（兼容 OpenCV 4.10）
                try:
                ...
                cv2.putText(cv_image, f'ID:{marker_id}', tuple(center),
                ...
                # 日志
                self.get_logger().info(f'📍 ID {marker_id}: X={t[0]:.3f} Y={t[1]:.3f} Z={t[2]:.3f} Yaw={np.degrees(yaw):.1f}°')
                self.detect_count += 1
        
        # 🔥【关键】无论是否检测到标记，都发布图像
        # 添加状态文字
        cv2.putText(cv_image, f'Frames: {self.frame_count} | Detects: {self.detect_count}', (10, 20),
                   cv2.FONT_HERSHEY_SIMPLEX, 0.4, (255, 255, 255), 1)
        cv2.putText(cv_image, f'Mode: {"FAR" if self.current_marker_id==33 else "NEAR"} (ID {self.current_marker_id})',
                   (10, 40), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (255, 255, 0), 1)
