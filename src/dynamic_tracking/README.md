...
`dynamic_tracking` 用于实机 ArUco 跟踪 PID 调参，提供：

- USB 相机 + ArUco 检测链路（复用 `tvec`）
- `dynamic_tracking_node`（X/Y/Yaw + Z 定高闭环）
- `dynamic_tracking_csv_logger_node`（CSV 记录 + 结束后指标汇总）

## 运行

```bash
ros2 launch dynamic_tracking dynamic_tracking.launch.py
```

你可直接按与 `pid_tuning` 相同风格覆写参数，例如：

```bash
ros2 launch dynamic_tracking dynamic_tracking.launch.py \
  use_rqt:=false \
  csv_prefix:=dynamic_tracking_xy_fine \
  kp_x:=0.6 ki_x:=0.02 kd_x:=0.02 \
  kp_y:=0.6 ki_y:=0.02 kd_y:=0.02 \
  kp_yaw:=0.60 ki_yaw:=0.02 kd_yaw:=0.03 \
  target_z:=2.5 kp_z:=0.60 ki_z:=0.0 kd_z:=0.03 \
  vx_limit:=1.0 vy_limit:=1.0 \
  enable_velocity_ff:=true ff_gain_x:=0.4 ff_gain_y:=0.4 \
  ff_filter_alpha:=0.4 ff_timeout_sec:=0.2 \
...
- 单次 CSV 前缀默认：`dynamic_tracking`
- 汇总 CSV 默认：`/home/zjh/project/rasip_pi_ws/log/tracking_csv/dynamic_tracking_summary.csv`
- 单次 CSV 实际文件示例：`/home/zjh/project/rasip_pi_ws/log/tracking_csv/dynamic_tracking_xy_fine_YYYYMMDD_HHMMSS.csv`
- 树莓派部署验证文件示例：`/home/zjh/project/rasip_pi_ws/log/tracking_csv/pi_dynamic_tracking_deploy_test_20260313_210208.csv`
...
默认已开启：结束时自动把本次 CSV 与 `dynamic_tracking_summary.csv` 复制到 `sync_target_dir`。
