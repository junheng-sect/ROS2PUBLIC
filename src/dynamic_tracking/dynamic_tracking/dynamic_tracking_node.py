...
class DynamicTrackingNode(Node):
    ...
    def __init__(self):
        ...
        # ===== 目标速度前馈（基于相对位姿差分） =====
        self.declare_parameter('enable_velocity_ff', True)
        self.declare_parameter('ff_gain_x', 0.4)
        self.declare_parameter('ff_gain_y', 0.4)
        self.declare_parameter('ff_filter_alpha', 0.4)
        self.declare_parameter('ff_timeout_sec', 0.2)
        ...
        self.ff_gain_x = float(self.get_parameter('ff_gain_x').value)
        self.ff_gain_y = float(self.get_parameter('ff_gain_y').value)
        self.ff_filter_alpha = float(self.get_parameter('ff_filter_alpha').value)
        self.ff_timeout_sec = float(self.get_parameter('ff_timeout_sec').value)
        ...
        self.get_logger().info(
            f'速度前馈 | enable={self.enable_velocity_ff} | '
            f'ff_gain_x={self.ff_gain_x:.3f} | ff_gain_y={self.ff_gain_y:.3f} | '
            f'ff_alpha={self.ff_filter_alpha:.3f} | ff_timeout={self.ff_timeout_sec:.3f}s'
        )
    ...
    def pose_callback(self, msg: ArucoBasePose):
        ...
        # 基于相对位姿差分估计相对速度，后续用于前馈补偿。
        if self.prev_pose_stamp is not None and self.prev_pose_x is not None and self.prev_pose_y is not None:
            dt = (now - self.prev_pose_stamp).nanoseconds / 1e9
            if dt > 1e-6:
                raw_vx = (float(msg.x) - self.prev_pose_x) / dt
                raw_vy = (float(msg.y) - self.prev_pose_y) / dt
                a = max(0.0, min(1.0, self.ff_filter_alpha))
                self.rel_vx_est = (1.0 - a) * self.rel_vx_est + a * raw_vx
                self.rel_vy_est = (1.0 - a) * self.rel_vy_est + a * raw_vy

        self.pose = msg
        self.has_pose = True
        self.last_pose_time = now
        self.prev_pose_x = float(msg.x)
        self.prev_pose_y = float(msg.y)
        self.prev_pose_stamp = now
    ...
    def is_ff_fresh(self) -> bool:
        """前馈估计是否新鲜，避免使用过期速度估计。"""
        if self.prev_pose_stamp is None:
            return False
        dt = (self.get_clock().now() - self.prev_pose_stamp).nanoseconds / 1e9
        return dt <= self.ff_timeout_sec
    ...
    def control_loop(self):
        ...
        # 目标速度前馈（相对位姿差分）：
        # 相对位姿定义下，v_rel = d(p_rel)/dt，若直接用于跟踪会表现为“目标相对运动”。
        # 使用 -v_rel 方向进行补偿可减轻纯 PID 的动态滞后。
        ff_vx = 0.0
        ff_vy = 0.0
        if self.enable_velocity_ff and self.is_ff_fresh():
            ff_vx = -self.ff_gain_x * self.rel_vx_est
            ff_vy = -self.ff_gain_y * self.rel_vy_est
            vx = self.clamp(vx + ff_vx, self.vx_limit)
            vy = self.clamp(vy + ff_vy, self.vy_limit)

        self.publish_body_velocity(vx, vy, vz, wz)
        self.latest_status = (
            f'OFFBOARD闭环 | ex={ex:.2f}, ey={ey:.2f}, ez={ez:.2f}, '
            f'eyaw={math.degrees(eyaw):.1f}deg | '
            f'vff=({ff_vx:.2f},{ff_vy:.2f}) | '
